...
const updates = {
  ...
  'reasoning': { en: 'REASONING', cn: '[破壁逻辑]', metaphor: '切断与外界的即时反应，在深沉绝对的黑暗脑海中，用上万条绝密线索进行惨烈的自我证伪与推演。', metaphor_en: `Severing instant reactions with the outside, using tens of thousands of top-secret clues to wage brutal self-falsification and deduction within the deep, absolute dark mind.` },
...
for (const [id, data] of Object.entries(updates)) {
  if (data.en) {
    let enRegex = new RegExp(`(id:\\s*'${id}'.*?en:\\s*')[^']+(',\\s*cn:\\s*')[^']+(')`);
    content = content.replace(enRegex, `$1${data.en}$2${data.cn}$3`);
  } else {
    let cnRegex = new RegExp(`(id:\\s*'${id}'.*?cn:\\s*')[^']+(')`);
    content = content.replace(cnRegex, `$1${data.cn}$2`);
  }

  let metaRegex = new RegExp(`(id:\\s*'${id}'[\\s\\S]*?metaphor:\\s*)'[^']+'(,\\s*metaphor_en:\\s*)'[^']+'`);
  let englishEscaped = data.metaphor_en.replace(/'/g, "\\\\'");
  content = content.replace(metaRegex, `$1'"${data.metaphor}"'$2'"${englishEscaped}"'`);
}
